...
export async function GET(
  ...
  try {
    ...
    if (config.pkce) {
      const verifier = req.cookies.get(OAUTH_COOKIES.codeVerifier(platform))?.value;
      if (!verifier) {
        return NextResponse.redirect(`${base}/social-media?error=missing_verifier`);
      }
    ...
    if (config.pkce) response.cookies.delete(OAUTH_COOKIES.codeVerifier(platform));
